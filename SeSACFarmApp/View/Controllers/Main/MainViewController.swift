...
/*
 ...
 4. 댓글 없을 때 가운데에 이미지나 레이블 하나 보여주기
 5. 작성 및 수정해서 들어가면 바로 키보드 올려주게 구현. -> 해결
 6. 빈값 으로 작성했을 때 체크. -> 해결
 7. 키보드노티피케이션 -> 해결
 8. 텍스트필드 내부 좌측 여백 -> 해결 
 9. Board 구조체 부분 -> 포스트작성자 id인지, 댓글 작성자id인지, 전부 똑같은 이름이어서 너무 헷갈림; 전부 다르게 구분 지어주기 -> 완료.
 10. 한국 시간으로 맞췄지만 시차가 여전히 존재. 개선 필요 11시간 차이 남.
 11. Token 갱신
 ...
 5. 글을 수정하는 뷰컨트롤러에서 글을 수정하고 완료한 뒤 pop으로 이전화면으로 돌아오면 수정된 글이 최신화되어 있지 않는 이슈 존재.
    ...
    해결: 새롭게 초기화한 뷰컨트롤러로 push해줬다. => 문제있음. 새롭게 초기화한 뷰컨트롤러로 푸쉬했더니 나머지 프로퍼티값들이 아무것도 안담겨있음. 개선 필요.
